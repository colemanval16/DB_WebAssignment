...
appropriate choice when developing your own database tables: 
</p>
    <ul>
    ...
    <p>Flat-type databases are simplistic in design. They are most commonly used in plain-text formats. Because their purpose is to hold one record per line, they make access, perfor- mance, and queries very quick. An example of this type of database would be what you would find in a .txt or .ini file. </p>
    <h5>Hierarchical databases</h5>
    <p>A hierarchical database is similar to a tree structure (such as a family tree). In this database, each “parent” table can have multiple “children,” but each child can have only one parent. </p>
    <h5>Relational databases</h5>
    <p>The last yet most important database type is the relational database. A relational database is similar to a hierarchical database in that data is stored in tables and any new information
is automatically added into the table without the need to reorganize the table itself. Unlike in hierarchical databases, however, a table in a relational database can have multiple parents. 
</p>
    <h5>Understanding Relational Database Concepts</h5>
        ...
        <ul>
            ...
            <li>A <strong>record</strong> is a data structure that is a collection of fields (elements), each with its own name and type that
                appear in a table as group of fields across one row.</li>
        </ul>
        ...
        <ul>
            <li>A <strong>one-to-one</strong> orelationship is an association between two tables in which the primary key value of each
            record in the primary table corresponds to the value in the matching field of one, and only one, record in the
            related table.</li>
            <li>A <strong>many-to-many</strong> relationship is a complex association between two sets of parameters in which many
            parameters of each set can relate to many others in the second set.</li>
            <li>A <strong>parent/child</strong> relationship is a relationship between nodes in a tree data structure in which the parent is
        ...
        <p><strong>Constraints</strong> are limitations or rules placed on a field or column to ensure that data that is
        considered invalid is not entered. For example, if a person’s age should be input, then the data
        that is entered must be a positive number—a person cannot have a negative age.</p>

        <ul>
            <li>Structured query language (<strong>SQL</strong>) is a relational database language used in querying, updating, and managing
        ...
        <h4>Understanding Data Definition Language(DDL)</h4>
                ...
                THE BOTTOM LINE SQL Server and can be used to create database objects such as tables and views. A schema is a description of a database to a database management system (DBMS) in the language provided
                by the DBMS.</p>
            <ul>
